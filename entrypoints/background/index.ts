...
} from "@/lib/api/tabReuse";
import { BUILTIN_TOOL_NAMES, executeTool } from "@/lib/api/llm";
...
export default defineBackground(() => {
  ...
  async function scheduleJob(payload: Record<string, unknown> = {}) {
      const { delaySeconds, timestamp, toolName, toolArgs, label, timeoutSeconds, mcpRegistry } = payload;
      const mcpSnapshot = buildScheduleMcpSnapshot(mcpRegistry as unknown[]);
